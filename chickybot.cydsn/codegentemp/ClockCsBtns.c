...
/*******************************************************************************
* Function Name: ClockCsBtns_Start
********************************************************************************
*
* Summary:
...
void ClockCsBtns_Start(void) 
{
    ...
    ClockCsBtns_CLKEN |= ClockCsBtns_CLKEN_MASK;
	ClockCsBtns_CLKSTBY |= ClockCsBtns_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ClockCsBtns_Stop
********************************************************************************
*
* Summary:
...
void ClockCsBtns_Stop(void) 
{
    ...
    ClockCsBtns_CLKEN &= (uint8)(~ClockCsBtns_CLKEN_MASK);
	ClockCsBtns_CLKSTBY &= (uint8)(~ClockCsBtns_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ClockCsBtns_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ClockCsBtns_CLKEN & ClockCsBtns_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ClockCsBtns_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ClockCsBtns_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ClockCsBtns__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ClockCsBtns_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ClockCsBtns_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ClockCsBtns_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ClockCsBtns_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ClockCsBtns_GetSourceRegister();
    uint16 oldDivider = ClockCsBtns_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ClockCsBtns_CLKEN & ClockCsBtns_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ClockCsBtns_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ClockCsBtns_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ClockCsBtns_DIV_PTR);
}


/*******************************************************************************
* Function Name: ClockCsBtns_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockCsBtns_MOD_SRC |= modeBitMask & (uint8)ClockCsBtns_MODE_MASK;
}


/*******************************************************************************
* Function Name: ClockCsBtns_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockCsBtns_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ClockCsBtns_MODE_MASK));
}


/*******************************************************************************
* Function Name: ClockCsBtns_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ClockCsBtns_MOD_SRC & (uint8)(ClockCsBtns_MODE_MASK);
}


/*******************************************************************************
* Function Name: ClockCsBtns_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ClockCsBtns_GetDividerRegister();
    uint8 oldSrc = ClockCsBtns_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ClockCsBtns_MOD_SRC |= CYCLK_SSS;
        ClockCsBtns_MOD_SRC =
            (ClockCsBtns_MOD_SRC & (uint8)(~ClockCsBtns_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ClockCsBtns_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ClockCsBtns_MOD_SRC & ClockCsBtns_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ClockCsBtns_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ClockCsBtns_PHASE = clkPhase & ClockCsBtns_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ClockCsBtns_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ClockCsBtns_PHASE & ClockCsBtns_PHASE_MASK;
}
