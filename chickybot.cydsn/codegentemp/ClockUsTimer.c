...
/*******************************************************************************
* Function Name: ClockUsTimer_Start
********************************************************************************
*
* Summary:
...
void ClockUsTimer_Start(void) 
{
    ...
    ClockUsTimer_CLKEN |= ClockUsTimer_CLKEN_MASK;
	ClockUsTimer_CLKSTBY |= ClockUsTimer_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ClockUsTimer_Stop
********************************************************************************
*
* Summary:
...
void ClockUsTimer_Stop(void) 
{
    ...
    ClockUsTimer_CLKEN &= (uint8)(~ClockUsTimer_CLKEN_MASK);
	ClockUsTimer_CLKSTBY &= (uint8)(~ClockUsTimer_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ClockUsTimer_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ClockUsTimer_CLKEN & ClockUsTimer_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ClockUsTimer_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ClockUsTimer_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ClockUsTimer__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ClockUsTimer_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ClockUsTimer_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ClockUsTimer_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ClockUsTimer_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ClockUsTimer_GetSourceRegister();
    uint16 oldDivider = ClockUsTimer_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ClockUsTimer_CLKEN & ClockUsTimer_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ClockUsTimer_DIV_PTR, clkDivider);
                ClockUsTimer_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ClockUsTimer_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ClockUsTimer_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ClockUsTimer_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ClockUsTimer_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ClockUsTimer_DIV_PTR);
}


/*******************************************************************************
* Function Name: ClockUsTimer_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockUsTimer_MOD_SRC |= modeBitMask & (uint8)ClockUsTimer_MODE_MASK;
}


/*******************************************************************************
* Function Name: ClockUsTimer_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockUsTimer_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ClockUsTimer_MODE_MASK));
}


/*******************************************************************************
* Function Name: ClockUsTimer_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ClockUsTimer_MOD_SRC & (uint8)(ClockUsTimer_MODE_MASK);
}


/*******************************************************************************
* Function Name: ClockUsTimer_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ClockUsTimer_GetDividerRegister();
    uint8 oldSrc = ClockUsTimer_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ClockUsTimer_MOD_SRC |= CYCLK_SSS;
        ClockUsTimer_MOD_SRC =
            (ClockUsTimer_MOD_SRC & (uint8)(~ClockUsTimer_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ClockUsTimer_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ClockUsTimer_MOD_SRC & ClockUsTimer_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ClockUsTimer_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ClockUsTimer_PHASE = clkPhase & ClockUsTimer_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ClockUsTimer_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ClockUsTimer_PHASE & ClockUsTimer_PHASE_MASK;
}
