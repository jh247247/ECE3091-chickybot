...
/*******************************************************************************
* Function Name: ClockLcdBacklight_Start
********************************************************************************
*
* Summary:
...
void ClockLcdBacklight_Start(void) 
{
    ...
    ClockLcdBacklight_CLKEN |= ClockLcdBacklight_CLKEN_MASK;
	ClockLcdBacklight_CLKSTBY |= ClockLcdBacklight_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ClockLcdBacklight_Stop
********************************************************************************
*
* Summary:
...
void ClockLcdBacklight_Stop(void) 
{
    ...
    ClockLcdBacklight_CLKEN &= (uint8)(~ClockLcdBacklight_CLKEN_MASK);
	ClockLcdBacklight_CLKSTBY &= (uint8)(~ClockLcdBacklight_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ClockLcdBacklight_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ClockLcdBacklight_CLKEN & ClockLcdBacklight_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ClockLcdBacklight_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ClockLcdBacklight_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ClockLcdBacklight__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ClockLcdBacklight_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ClockLcdBacklight_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ClockLcdBacklight_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ClockLcdBacklight_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ClockLcdBacklight_GetSourceRegister();
    uint16 oldDivider = ClockLcdBacklight_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ClockLcdBacklight_CLKEN & ClockLcdBacklight_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ClockLcdBacklight_DIV_PTR, clkDivider);
                ClockLcdBacklight_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ClockLcdBacklight_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ClockLcdBacklight_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ClockLcdBacklight_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ClockLcdBacklight_DIV_PTR, clkDivider);
				ClockLcdBacklight_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ClockLcdBacklight_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ClockLcdBacklight_DIV_PTR);
}


/*******************************************************************************
* Function Name: ClockLcdBacklight_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockLcdBacklight_MOD_SRC |= modeBitMask & (uint8)ClockLcdBacklight_MODE_MASK;
}


/*******************************************************************************
* Function Name: ClockLcdBacklight_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockLcdBacklight_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ClockLcdBacklight_MODE_MASK));
}


/*******************************************************************************
* Function Name: ClockLcdBacklight_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ClockLcdBacklight_MOD_SRC & (uint8)(ClockLcdBacklight_MODE_MASK);
}


/*******************************************************************************
* Function Name: ClockLcdBacklight_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ClockLcdBacklight_GetDividerRegister();
    uint8 oldSrc = ClockLcdBacklight_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ClockLcdBacklight_MOD_SRC |= CYCLK_SSS;
        ClockLcdBacklight_MOD_SRC =
            (ClockLcdBacklight_MOD_SRC & (uint8)(~ClockLcdBacklight_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ClockLcdBacklight_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ClockLcdBacklight_MOD_SRC & ClockLcdBacklight_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ClockLcdBacklight_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ClockLcdBacklight_PHASE = clkPhase & ClockLcdBacklight_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ClockLcdBacklight_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ClockLcdBacklight_PHASE & ClockLcdBacklight_PHASE_MASK;
}
