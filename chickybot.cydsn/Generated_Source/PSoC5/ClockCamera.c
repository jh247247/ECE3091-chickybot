...
/*******************************************************************************
* Function Name: ClockCamera_Start
********************************************************************************
*
* Summary:
...
void ClockCamera_Start(void) 
{
    ...
    ClockCamera_CLKEN |= ClockCamera_CLKEN_MASK;
	ClockCamera_CLKSTBY |= ClockCamera_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ClockCamera_Stop
********************************************************************************
*
* Summary:
...
void ClockCamera_Stop(void) 
{
    ...
    ClockCamera_CLKEN &= (uint8)(~ClockCamera_CLKEN_MASK);
	ClockCamera_CLKSTBY &= (uint8)(~ClockCamera_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ClockCamera_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ClockCamera_CLKEN & ClockCamera_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ClockCamera_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ClockCamera_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ClockCamera__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ClockCamera_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ClockCamera_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ClockCamera_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ClockCamera_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ClockCamera_GetSourceRegister();
    uint16 oldDivider = ClockCamera_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ClockCamera_CLKEN & ClockCamera_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ClockCamera_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ClockCamera_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ClockCamera_DIV_PTR);
}


/*******************************************************************************
* Function Name: ClockCamera_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockCamera_MOD_SRC |= modeBitMask & (uint8)ClockCamera_MODE_MASK;
}


/*******************************************************************************
* Function Name: ClockCamera_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockCamera_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ClockCamera_MODE_MASK));
}


/*******************************************************************************
* Function Name: ClockCamera_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ClockCamera_MOD_SRC & (uint8)(ClockCamera_MODE_MASK);
}


/*******************************************************************************
* Function Name: ClockCamera_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ClockCamera_GetDividerRegister();
    uint8 oldSrc = ClockCamera_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ClockCamera_MOD_SRC |= CYCLK_SSS;
        ClockCamera_MOD_SRC =
            (ClockCamera_MOD_SRC & (uint8)(~ClockCamera_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ClockCamera_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ClockCamera_MOD_SRC & ClockCamera_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ClockCamera_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ClockCamera_PHASE = clkPhase & ClockCamera_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ClockCamera_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ClockCamera_PHASE & ClockCamera_PHASE_MASK;
}
