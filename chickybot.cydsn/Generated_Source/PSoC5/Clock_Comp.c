...
/*******************************************************************************
* Function Name: Clock_Comp_Start
********************************************************************************
*
* Summary:
...
void Clock_Comp_Start(void) 
{
    ...
    Clock_Comp_CLKEN |= Clock_Comp_CLKEN_MASK;
	Clock_Comp_CLKSTBY |= Clock_Comp_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Comp_Stop
********************************************************************************
*
* Summary:
...
void Clock_Comp_Stop(void) 
{
    ...
    Clock_Comp_CLKEN &= (uint8)(~Clock_Comp_CLKEN_MASK);
	Clock_Comp_CLKSTBY &= (uint8)(~Clock_Comp_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Comp_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Comp_CLKEN & Clock_Comp_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Comp_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Comp_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Comp__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Comp_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Comp_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Comp_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Comp_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Comp_GetSourceRegister();
    uint16 oldDivider = Clock_Comp_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Comp_CLKEN & Clock_Comp_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Comp_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Comp_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Comp_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Comp_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Comp_MOD_SRC |= modeBitMask & (uint8)Clock_Comp_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Comp_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Comp_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Comp_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Comp_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Comp_MOD_SRC & (uint8)(Clock_Comp_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Comp_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Comp_GetDividerRegister();
    uint8 oldSrc = Clock_Comp_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Comp_MOD_SRC |= CYCLK_SSS;
        Clock_Comp_MOD_SRC =
            (Clock_Comp_MOD_SRC & (uint8)(~Clock_Comp_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Comp_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Comp_MOD_SRC & Clock_Comp_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Comp_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Comp_PHASE = clkPhase & Clock_Comp_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Comp_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Comp_PHASE & Clock_Comp_PHASE_MASK;
}
