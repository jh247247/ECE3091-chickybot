...
uint8 Comp_Elbow_initVar = 0u;

/* Internal functions definitoin */
static void Comp_Elbow_trimAdjust(uint8 nibble) ;

/* static Comp_Elbow_backupStruct  Comp_Elbow_backup; */
#if (CY_PSOC5A)
    static Comp_Elbow_LOWPOWER_BACKUP_STRUCT  Comp_Elbow_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 Comp_Elbow_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: Comp_Elbow_Init
********************************************************************************
*
* Summary:
...
void Comp_Elbow_Init(void) 
{
    ...
    Comp_Elbow_SetSpeed(Comp_Elbow_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( Comp_Elbow_DEFAULT_HYSTERESIS == 0u )
        Comp_Elbow_CR |= Comp_Elbow_HYST_OFF;
    #else
        Comp_Elbow_CR &= (uint8)(~Comp_Elbow_HYST_OFF);
    #endif /* Comp_Elbow_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( Comp_Elbow_DEFAULT_PWRDWN_OVRD == 0u )
            Comp_Elbow_CR &= (uint8)(~Comp_Elbow_PWRDWN_OVRD);
        #else 
            Comp_Elbow_CR |= Comp_Elbow_PWRDWN_OVRD;
        #endif /* Comp_Elbow_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    Comp_Elbow_CR |= Comp_Elbow_MX_AO;

    /* Set default sync */
    Comp_Elbow_CLK &= (uint8)(~Comp_Elbow_SYNCCLK_MASK);
    #if ( Comp_Elbow_DEFAULT_BYPASS_SYNC == 0u )
        Comp_Elbow_CLK |= Comp_Elbow_SYNC_CLK_EN;
    #else
        Comp_Elbow_CLK |= Comp_Elbow_BYPASS_SYNC;
    #endif /* Comp_Elbow_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: Comp_Elbow_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_Elbow_PWRMGR |= Comp_Elbow_ACT_PWR_EN;
    Comp_Elbow_STBY_PWRMGR |= Comp_Elbow_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(Comp_Elbow_restoreReg == 1u)
        {
            Comp_Elbow_CR = Comp_Elbow_lowPowerBackup.compCRReg;

            /* Clear the flag */
            Comp_Elbow_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: Comp_Elbow_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Comp_Elbow_initVar: Is modified when this function is called for the 
...
void Comp_Elbow_Start(void) 
{
    ...
    if ( Comp_Elbow_initVar == 0u )
    {
        Comp_Elbow_Init();
        
        Comp_Elbow_initVar = 1u;
    }   
...
void Comp_Elbow_Stop(void) 
{
    ...
    Comp_Elbow_PWRMGR &= (uint8)(~Comp_Elbow_ACT_PWR_EN);
    Comp_Elbow_STBY_PWRMGR &= (uint8)(~Comp_Elbow_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        Comp_Elbow_restoreReg = 1u;

        /* Save the control register before clearing it */
        Comp_Elbow_lowPowerBackup.compCRReg = Comp_Elbow_CR;
        Comp_Elbow_CR = Comp_Elbow_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: Comp_Elbow_SetSpeed
********************************************************************************
*
* Summary:
...
void Comp_Elbow_SetSpeed(uint8 speed) 
{
    ...
    Comp_Elbow_CR = (Comp_Elbow_CR & (uint8)(~Comp_Elbow_PWR_MODE_MASK)) |
                           (speed & Comp_Elbow_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == Comp_Elbow_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Elbow_TR = Comp_Elbow_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Elbow_TR0 = Comp_Elbow_HS_TRIM_TR0;
            Comp_Elbow_TR1 = Comp_Elbow_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            Comp_Elbow_TR = Comp_Elbow_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            Comp_Elbow_TR0 = Comp_Elbow_LS_TRIM_TR0;
            Comp_Elbow_TR1 = Comp_Elbow_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: Comp_Elbow_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( Comp_Elbow_WRK & Comp_Elbow_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: Comp_Elbow_trimAdjust
********************************************************************************
*
* Summary:
...
static void Comp_Elbow_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = Comp_Elbow_WRK & Comp_Elbow_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Elbow_TR |= Comp_Elbow_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Elbow_TR0 |= Comp_Elbow_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                Comp_Elbow_TR |= Comp_Elbow_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                Comp_Elbow_TR1 |= Comp_Elbow_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((Comp_Elbow_WRK & Comp_Elbow_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: Comp_Elbow_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 Comp_Elbow_ZeroCal(void) 
{
    ...
    tmpSW0 = Comp_Elbow_SW0;
    tmpSW2 = Comp_Elbow_SW2;
    tmpSW3 = Comp_Elbow_SW3;

     /* Clear routing for inP, retain routing for inN */
    Comp_Elbow_SW0 = 0x00u;
    Comp_Elbow_SW2 = 0x00u;
    Comp_Elbow_SW3 = tmpSW3 & (uint8)(~Comp_Elbow_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = Comp_Elbow_CR;
    Comp_Elbow_CR |= (Comp_Elbow_CAL_ON | Comp_Elbow_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        Comp_Elbow_TR = Comp_Elbow_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        Comp_Elbow_TR0 = Comp_Elbow_DEFAULT_CMP_TRIM;
        Comp_Elbow_TR1 = Comp_Elbow_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (Comp_Elbow_CR & Comp_Elbow_PWR_MODE_MASK) == Comp_Elbow_PWR_MODE_FAST)
    {
        Comp_Elbow_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        Comp_Elbow_trimAdjust(1u);
		Comp_Elbow_trimAdjust(0u);
    }
    ...
    Comp_Elbow_SW0 = tmpSW0;
    Comp_Elbow_SW2 = tmpSW2;
    Comp_Elbow_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) Comp_Elbow_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)Comp_Elbow_TR1 << 8u) | (Comp_Elbow_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: Comp_Elbow_LoadTrim
********************************************************************************
*
* Summary:
...
void Comp_Elbow_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: Comp_Elbow_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comp_Elbow_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
